...
mod tests {
    ...
    use crate::gsmtap::{GsmtapHeader, GsmtapMessage, GsmtapType, LteNasSubtype, UmSubtype};
    ...
    fn test_parsed_non_nas_element_advances_clock_and_warns() {
        let mut analyzer = NoNasMessagesAnalyzer::new();
        // This fork's 2G element carries the message bytes, so build one the
        // way the parser would.
        let non_nas = InformationElement::GSM(Box::new(GsmInformationElement {
