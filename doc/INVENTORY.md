...
4. **`incomplete_sib` presentation:** informational-only in code, presented as a warning-producer in the UI with no `informational` tag. Fix the UI entry, the code's severity, or just document the reality? *Docs status: `detectors/incomplete-sib.md` documents the reality (informational-only, invisible alone) and flags the UI discrepancy for maintainers. Underlying code/UI fix still your call.*
5. **RRLP has no dedup** while both LPP analyzers dedup per transaction. Intentional (2G sessions expected rare/short) or an oversight worth fixing before its detector page freezes the behavior in writing? *Docs status: `detectors/rrlp.md` documents the no-dedup behavior and carries a `NEEDS INPUT` marker on this exact question.*
6. **PinePhone page:** present it as upstream does (PinePhone/Pro via headless profile), or document this fork's reality that the profile is also the EG25-G modem port under active work? Affects `pinephone.md` and `porting.md`. *Docs status: `devices/index.md` (new) treats PinePhone as headless per upstream; the inherited `pinephone.md` was left untouched. Still open.*
7. **`auto_check_updates` default mismatch:** `false` in the shipped template, `true` in code (so any config missing the key gets update checks). Which is intended? *Docs status: `configuration-reference.md` prints both and flags the mismatch rather than picking one. Needs a code/template decision.*
8. **Rebase timing vs. imsi-requested page:** upstream's home-PLMN-from-SIM work (landed today) changes that detector's false-positive logic. *Docs status: `detectors/imsi-requested.md` written against current fork code, with a note that upstream drift will change the roaming logic on next rebase. Revise the page after rebasing.*
9. ~~**`imsi_requested` real-world validation**~~ **ANSWERED 2026-08-31 (per maintainer, hedged "I believe"):** EFF validated it against a real cell-site simulator, using a test device obtained through a university research group. Documented in `detectors/imsi-requested.md`, `validation.md`, and the index, framed as a true-positive validation (it fires on a real catcher) distinct from a field false-positive rate (still unknown). NEEDS INPUT marker removed. *To confirm before public hosting: the exact provenance wording, and whether a connecting party should be named (the maintainer mentioned one, transcribed unclearly as "Cape Mobile"; omitted from the text pending confirmation).*
10. **Do we have any real recording containing LPP traffic?** Re-analyze existing captures with the LPP analyzers on and see whether any carrier LPP shows up. *Docs status: `detectors/lpp.md` and `validation.md` state "never confirmed against real traffic." A real capture would let those be upgraded.*
11. **`UPSTREAM.md` subscriber-identity entry** (§5.1): I can draft it — confirm and it lands alongside the docs work. *Still open; not yet drafted.*

### New during the docs run
12. **Concurrent `timing_advance` detector + `sim_health` work** appeared uncommitted in the tree during writing (a new 12th detector, "A tower that seems to have moved," plus SIM-health). Left entirely untouched and excluded from all docs commits. Once committed it needs: a `doc/detectors/timing-advance.md` page, a row in `detectors/index.md` (currently lists 11), a `SUMMARY.md` entry, and a `configuration-reference.md` key. Its `heuristics.ts` summary is written; match the page's one-sentence summary to it.
13. **Banned words in inherited upstream pages.** `STYLE.md` §4 bans simply/just/obviously/easy. Nine instances remain in inherited pages not rewritten this run: `heuristics.md`, `updating-rayhunter.md`, `porting.md`, `references.md`, `introduction.md` (also has marketing-voice "as easy to install as possible"), `moxee.md`. Left for a deliberate inherited-page pass, since rewriting upstream tone is a judgment call.
